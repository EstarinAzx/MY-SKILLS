...
2. **Structural pass** (deterministic, free):
   ...
   Add `--stale [days]` to also flag freshness: `stale` (page older than `days`, default 90, by its `updated:` date) and `source-newer` (a `raw/` source modified after the page derived from it). Use it on a periodic health-check, not every lint.
