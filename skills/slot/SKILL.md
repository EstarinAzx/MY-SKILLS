...
`wisp routing` needs wisp-router newer than 2.0.10. If the installed global lacks it (`wisp routing` opens the TUI instead of printing), run the source entry from the Wisp repo checkout — on this machine `D:/.claude/claude projects/autocomplete_extension`; do not search the filesystem for it:

```
cd "D:/.claude/claude projects/autocomplete_extension"
...
**2. Lease check.** If `C:/Users/S.D/.claude/slot/lease.json` exists, STOP. Do not rebind, spawn, or overwrite. Surface its contents. Only after the user confirms no Slot-driven agent is still running: restore from the lease (step 8 semantics), verify, delete it — then start fresh.

**3. Snapshot + write lease.** Read `wisp routing --json`, extract the Slot family's current entry, and write `C:/Users/S.D/.claude/slot/lease.json` BEFORE any mutation:
...
**9. Verify + delete lease.** Re-read routing. Slot matches the saved prior state → delete `C:/Users/S.D/.claude/slot/lease.json`. Mismatch → keep the lease and report.
