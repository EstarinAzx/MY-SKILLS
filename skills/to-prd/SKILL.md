...
4. After the issue is created, invoke `/context-sync-update` if `.context/` exists in the project. The PRD's implementation and testing decisions are exactly what `.context/decisions.md` captures, and the new issue becomes the "Pick up here" pointer in `active-work.md` for the next agent.
