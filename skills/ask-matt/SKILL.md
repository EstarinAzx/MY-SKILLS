...
> **Local ecosystem note:** this install is curated. `/tdd` → use the
> superpowers:test-driven-development skill. `/handoff` → use /context-update
> (context-sync). `/code-review` → the built-in code-review skill.
> `diagnosing-bugs` → superpowers:systematic-debugging (or bugs-begone).
