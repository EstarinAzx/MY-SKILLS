...
After all issues are created, invoke `/context-sync-update` if `.context/` exists in the project. The breakdown decisions and the issue list are exactly the kind of state the next agent will want — record them in `.context/active-work.md` and `.context/decisions.md`.
